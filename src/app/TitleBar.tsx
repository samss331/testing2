...
import { TernaryProSuccessDialog } from "@/components/TernaryProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showTernaryProSuccessDialog = () => {
...
export function TernaryProButton({
  isTernaryProEnabled,
}: {
  ...
  return (
    <Button
      data-testid="title-bar-ternary-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isTernaryProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isTernaryProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isTernaryProEnabled && (
