import { isTernaryProEnabled, type LargeLanguageModel } from "@/lib/schemas";
...
export function ModelPicker() {
  ...
  const isSmartAutoEnabled =
    settings.enableProSmartFilesContextMode && isTernaryProEnabled(settings);
  ...
  if (settings && isTernaryProEnabled(settings)) {
          ...
          <>
            ...
            {primaryProviders.map(([providerId, models]) => {
              models = models.filter((model) => {
                // Don't show free models if Ternary Pro is enabled because
                // we will use the paid models (in Ternary Pro backend) which
