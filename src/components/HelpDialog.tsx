...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const isTernaryProUser = settings?.providerSettings?.["auto"]?.apiKey?.value;
...
`;
      ...
      const githubIssueUrl = `https://github.com/ternarystudio/ternary/issues/new?title=${encodedTitle}&labels=bug,filed-from-app&body=${encodedBody}`;
      ...
      IpcClient.getInstance().openExternalUrl(
        "https://github.com/ternarystudio/ternary/issues/new",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://ternary-pre-domain.vercel.appapi/log-uploads/generate-upload-url",
        {
          method: "POST",
          headers: {
...
`;
    ...
    const githubIssueUrl = `https://github.com/ternarystudio/ternary/issues/new?title=${encodedTitle}&labels=support&body=${encodedBody}`;
        ...
        <div className="flex flex-col space-y-4 w-full">
          {isTernaryProUser ? (
            <div className="flex flex-col space-y-2">
              ...
              >
                <SparklesIcon className="mr-2 h-5 w-5" /> Chat with Ternary help
                bot (Pro)
              </Button>
