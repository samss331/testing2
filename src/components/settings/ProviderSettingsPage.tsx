...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  // Use fetched data (or defaults for Ternary)
  const providerDisplayName = isTernary
    ? "Ternary"
    : (providerData?.name ?? "Unknown Provider");
  const providerWebsiteUrl = isTernary
    ? "https://ternary-pre-domain.vercel.app/dashbord"
    : providerData?.websiteUrl;
  const hasFreeTier = isTernary ? false : providerData?.hasFreeTier;
  const envVarName = isTernary ? undefined : providerData?.envVarName;
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      if (isTernary) {
        settingsUpdate.enableTernaryPro = true;
      }
  ...
  // --- Toggle Ternary Pro Handler ---
  const handleToggleTernaryPro = async (enabled: boolean) => {
    setIsSaving(true);
    try {
      await updateSettings({
        enableTernaryPro: enabled,
      });
    } catch (error: any) {
      showError(`Error toggling Ternary Pro: ${error}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen px-8 py-4">
      <div className="max-w-4xl mx-auto">
        ...
        {isTernary && !settingsLoading && (
          <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
            <div>
              <h3 className="font-medium">Enable Ternary Pro</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Toggle to enable Ternary Pro
              </p>
            </div>
            <Switch
              checked={settings?.enableTernaryPro}
              onCheckedChange={handleToggleTernaryPro}
              disabled={isSaving}
            />
          </div>
        )}
